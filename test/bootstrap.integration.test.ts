 ...
 * calls (`pane run`). The workbench is now just two named tabs — serve (API +
 * engine loop) and board (TUI) — with no orchestrator agent.
 */
let server: Server;
let dir: string;
...
const OPTS = {
  ...
  board: true,
  serveCommand: "/repo/bin/faktory serve fk --no-board --port 4600",
};
...
test("fresh detached bootstrap puts serve + board each in a named tab", async () => {
  let nextTab = 2;
  let nextPane = 2;
  handler = (method) => {
    switch (method) {
      ...
      case "pane.list":
        return {
          panes: requests.filter((r) => r.method === "workspace.create").length
  ...
  assert.equal(result.boardPaneId, "ws1:p2");
  ...
  assert.deepEqual(tabLabels, [TAB_LABELS.board]);

  assert.ok(!requests.some((r) => r.method === "pane.split"), "must not split panes");
  assert.ok(!requests.some((r) => r.method === "agent.start"), "no orchestrator agent");

  const calls = herdrCalls();
  assert.ok(calls.some((c) => c.includes("pane run ws1:p1") && c.includes("serve fk")));
  assert.ok(calls.some((c) => c.includes("pane run ws1:p2") && c.includes("tui --config fk")));
});

test("reattach leaves a fully-running workbench untouched", async () => {
      ...
      case "tab.list":
        return {
          tabs: [
            ...
            { tab_id: "ws1:t2", label: TAB_LABELS.board },
          ],
        };
      case "pane.process_info": {
        const cmd =
          params.pane_id === "ws1:p1"
            ? "node /repo/src/cli.ts serve --config fk"
            : "node /repo/src/cli.ts tui --config fk";
...
test("reattach restarts a dead board by reusing its idle named tab", async () => {
  handler = (method, params) => {
    switch (method) {
      case "workspace.list":
        return { workspaces: [{ workspace_id: "ws1", label: "faktory:fk" }] };
      ...
      case "pane.process_info": {
        if (params.pane_id === "ws1:p1")
          return {
            process_info: { shell_pid: 1, foreground_processes: [{ pid: 2, cmdline: "node /repo/src/cli.ts serve --config fk" }] },
          };
  ...
  assert.equal(result.boardPaneId, "ws1:p2", "reuses the existing board tab's pane");
...
test("adopt branch resolves the root tab id via pane.get when pane.list omits it", async () => {
  ...
  const result = await bootstrapDetached(client, { ...OPTS, board: false });
...
test("attached bootstrap labels the serve tab and opens the board tab", async () => {
  let nextTab = 2;
  let nextPane = 2;
  handler = (method, params) => {
    switch (method) {
  ...
  const result = await bootstrapWorkbench(client, { ...OPTS, fromPaneId: "ws1:p1", serveTab: true });
