# AGENTS.md — Faktory contributor guide (for agents)

Faktory is evolved primarily by coding agents. Read this before changing code.

## What Faktory is

A local orchestration system that drives coding agents inside herdr from an
...
- **Tests are the spec.** `pnpm test` runs unit + integration tests (fake
  Notion server, fake herdr socket, real ephemeral SQLite + real API server on
...
- **`terminal-ui`** (pproenca/dot-skills) — for anything under `src/tui` or CLI
  output. Key rules we already apply and you must preserve: compose each frame
  off-screen and write it in a **single** write; **overwrite** lines (`ESC[K`)
  instead of clearing the screen; always provide **escape routes** (q / esc /
  ctrl+c); give immediate feedback for every keypress; use colors semantically
  (red=blocked/failed, green=done/deployable, yellow=in-flight).

To refresh or add skills use the skills CLI, and keep them vendored in-repo:

```sh
npx skills add vercel-labs/agent-skills@web-design-guidelines -y
npx skills add pproenca/dot-skills@terminal-ui -y
npx skills check   # update check
```
...
# Architecture guideline (adopted)

The section below is the [Domain-Driven Hexagon](https://github.com/Sairyss/domain-driven-hexagon)
guideline, vendored as Faktory's reference architecture. Apply it with
judgement: Faktory is small, so we use the *principles* (hexagonal boundaries,
explicit ports, domain purity, testing pyramid) — not every ceremony.
