...
| POST   | `/api/tasks/:id/transition` | `{ to, actor?, note? }`                         | validated lifecycle move, mirrored to the source (tags + status). `409` on illegal moves, and on any move into `queued` when the task has unmet dependencies (body includes `blockers`). |
...
/api/tasks/:id/transition` to `queued` (whether promoting a `discovered` task
or reviving a `failed`/`cancelled` one) while any dependency is unsatisfied
