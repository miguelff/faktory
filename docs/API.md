...
| GET    | `/api/errors`               | — (`?all=1` to include resolved)                | the local error log (inconsistencies never stored in the datasource); `{ errors: ErrorEntry[] }` newest first, open-only by default |
| POST   | `/api/errors/:id/resolve`   | —                                               | mark one open error resolved; `{ ok }`. `404` when there is no open error with that id |
...
| POST   | `/api/tasks/:id/transition` | `{ to, actor?, note?, force? }`                 | **manual repair only** — validated lifecycle move written through to the datasource (outbox: acknowledged, then projected); `force` bypasses validation (still audited, still written through). `409` on illegal moves or a lost CAS, `400` on a bad phase. |
