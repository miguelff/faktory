...
- ☑ **CLI command grammar** — the whole surface now reads as `<resource>
  <verb>`: task operations moved under a `task` group (`task sync`, `task
  list`, `task transition`) to match `config <verb>` / `source <verb>`. The
